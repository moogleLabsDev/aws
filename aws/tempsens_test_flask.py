...
import time
import pandas as pd
from flask import Flask, request, render_template, session, redirect,send_from_directory,send_file ,Response
...
def convert ():
	if request.method=='POST':
		...
		def support (file):
			...
			df2=pd.read_csv('templates/min_nearby_distances.csv')
			df1=pd.read_csv(file)
			df2 = df2[df2['Device_id3'].notna()]
			df2 = df2[df2['Device_id2'].notna()]
			return df1,df2

		df1,df2=support(file)
		...
		def model1 ():
			...
			MODEL_PATH = 'templates/all_4_inputs_4_nodes_50000.pth'
		...
		dataout.to_csv("templates/results.csv")
		#print(dataout)
		
		return render_template("result.html" ,tables=[dataout.to_html()], titles=dataout.columns.values)
		
	return render_template('home.html')

@app.route("/getCSV")
def getCSV():
	with open("templates/results.csv") as fp:
		csv = fp.read()
